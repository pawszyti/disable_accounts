...
<table class="tabela2" cellspacing='0'>
    ...
    for ($i = 0; $i < $ilosc; $i++)
    {
        ...
        if ($tablica['r_status']==no)    {
            echo "<td><input type=submit class=\"btn\" value='usuń' onclick=\"bootbox.confirm('Czy chcesz z listy usunąć użytkownika<b> ".$tablica['r_imie']." ".$tablica['r_nazwisko']."</b> ?', function(result){ if (result==true) {window.location.href='del.php?id=".$tablica['r_id']."'}; });\" class=\"myButton2\"></td>";
        }
        else
        {
            echo "<td> </td>";
